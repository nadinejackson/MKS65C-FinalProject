...
{
  int r;
  int thing = open("./set.game", O_RDONLY, 0666);
...
int view(int * locations)
{
  int thing = open("./set.game", O_RDONLY);
  perror("error opening set file");
  int * place;
  read(thing, place, sizeof(int));
  read(thing, locations, 15 * sizeof(int));
  return *place;
}
